...
class OtsuThresholder:
    ...
    def __call__(self, image):
        ...
        if self.output_process: cv2.imwrite('output/thresholded.jpg', hist)
...
class Closer:
    def __init__(self, kernel_size = 5, iterations = 10, output_process = False):
...
class EdgeDetector:
    ...
    def __call__(self, image, thresh1 = 50, thresh2 = 150, apertureSize = 3):
        min_val,max_val = self.calc_threshold(image)
        edges = cv2.Canny(image, min_val, max_val, apertureSize = apertureSize)
        kernel = np.ones((5,5),np.uint8)
